...
- 実験の通し番号`n`は，軽微な修正（ハイパーパラメータの変更，比較手法の追加など）であればこれを維持し，モデル構造やデータの前処理，誤差関数等の問題の本質が変わる変更がある場合は遠し番号を`n+1`に変更する．
- 軽微な変更の場合 `{n:03}{m}`のように小さな通し番号を追加する．
...
- なお，実験的評価のために実行したプログラムの学習結果もここに保存する．特に，レポートで実験的評価に言及する際の根拠になるためこれの保存は重要である．
- 実験において誤差や精度を計測しこれを保存する際は，学習用，検証用の両方を必ず保存する．人間が結果から必要な評価を抽出するため，AIエディタの判断で記録する値を決定してはいけない．
- モデルの学習が終了した時点でモデルの最終エポックにおける重みと，最適化手法の内部状態をファイルに保存する．これは，実験後にエポック数を増やし再実験する際に，学習履歴を引き継ぎ効率化するための施策である．そのため，状況に応じ保存するファイルを判断する．
- 同じプログラムで実験条件を変更し再度実行することを指示された場合，各オーダーにおける実験条件をコメントとして保持する
...
trainやepoch，iteration関数が最適化手法によって処理が分岐する場合，isinstance関数でoptimizerのクラスを調べ，if文で分岐してください．** train，epoch，iteration関数は指示通りのフォーマットに準拠してください **
